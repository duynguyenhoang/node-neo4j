...
function isLikeSearchValue(obj) {
  var str = '.*';
  var lastIndex = obj.lastIndexOf(str);
  var isLastIndex = (lastIndex !== -1) && (lastIndex + str.length === obj.length);
  var isFirstIndex = obj.indexOf(str) === 0;

  return isLastIndex && isFirstIndex;
}
/*  Example:
...
function whereSetProperties (fieldName, oldProps, newProps) {
  ...
  for (var k in oldProps) {
    ...
    if (k === idFunction) {
      whereClause += 'ID(' + originalFieldName + ')=' + obj;
    } else {
